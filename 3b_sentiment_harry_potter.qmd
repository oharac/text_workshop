...
# Sentiment analysis example: Harry Potter and the Sorceror's Stone
...
We will use `str_detect()` to look for any cells in "text_full" column that contains the string "CHAPTER", and if it does, the new column will contain that chapter number (by word):
